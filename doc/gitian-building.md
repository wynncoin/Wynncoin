...
*Setup instructions for a Gitian build of Wynn Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Wynn
...
- [Building Wynn Core](#building-wynn-core)
...
Clone the git repositories for wynn and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/wynnproject/wynn --recursive
git clone https://github.com/wynnproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/wynn/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the wynn repository under 'Fetch and create inputs' to install sources which require
...
To build Wynn Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the wynn repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/wynn/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/wynnproject/wynn
...
For example:
```bash
URL=https://github.com/laanwj/wynn.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit wynn=${COMMIT} --url wynn=${URL} ../wynn/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit wynn=${COMMIT} --url wynn=${URL} ../wynn/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit wynn=${COMMIT} --url wynn=${URL} ../wynn/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-wynn:
./bin/gbuild --commit wynn=${COMMIT},cpp-eth-wynn=${ETHCOMMIT} --url wynn=${URL},cpp-eth-wynn=${ETHURL} ../wynn/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the wynn git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../wynn/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/wynnproject/wynn-detached-sigs.git

BTCPATH=/some/root/path/wynn
SIGPATH=/some/root/path/wynn-detached-sigs

./bin/gbuild --url wynn=${BTCPATH},signature=${SIGPATH} ../wynn/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/wynn-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/wynn-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/wynn-osx-build.assert
```
...
[wynnproject/gitian.sigs](https://github.com/wynnproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@wynn.org) and he will commit them.