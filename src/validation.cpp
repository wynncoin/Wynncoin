...
#if defined(NDEBUG)
# error "Wynn cannot be compiled without assertions."
#endif
...
std::unique_ptr<WynnState> globalState;
...
const std::string strMessageMagic = "Wynn Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = wynnDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = wynnDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            WynnTxConverter converter(tx, NULL);
            ExtractWynnTX resultConverter;
            if(!converter.extractionWynnTransactions(resultConverter)){
            ...
            std::vector<WynnTransaction> wynnTransactions = resultConverter.first;
            std::vector<EthTransactionParams> wynnETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(WynnTransaction wynnTransaction : wynnTransactions){
                sumGas += wynnTransaction.gas() * wynnTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, wynnTransaction.gasPrice());
                } else {
                    txMinGasPrice = wynnTransaction.gasPrice();
                }
                VersionVM v = wynnTransaction.getVersion();
                if(v.format!=0)
                ...
                if(wynnTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(wynnTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(wynnTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)wynnTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(wynnETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > wynnTransactions.size())
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s WYC additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // wynn
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // wynn
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    WynnTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<WynnTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path wynnDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(wynnDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(wynnDir.string(), std::ios::in | std::ios::out);
...
bool WynnTxConverter::extractionWynnTransactions(ExtractWynnTX& wynntx){
    std::vector<WynnTransaction> resultTX;
    ...
    wynntx = std::make_pair(resultTX, resultETP);
    return true;
}

bool WynnTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool WynnTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
WynnTransaction WynnTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    WynnTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = WynnTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = WynnTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // wynn
    WynnDGP wynnDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setWynnSchedule(wynnDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = wynnDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = wynnDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = wynnDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // wynn
...
///////////////////////////////////////////////////////////////////////////////////////// wynn
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            WynnTxConverter convert(tx, &view, &block.vtx);

            ExtractWynnTX resultConvertWynnTX;
            if(!convert.extractionWynnTransactions(resultConvertWynnTX)){
            ...
            if(!CheckMinGasPrice(resultConvertWynnTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertWynnTX.first, blockGasLimit);
            ...
            for(WynnTransaction& qtx : resultConvertWynnTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertWynnTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertWynnTX.first[k].from(), resultConvertWynnTX.first[k].to(),
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // wynn
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // wynn
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // wynn
            globalState->setRootUTXO(oldHashUTXORoot); // wynn
...
public:
    ...
    CHashViewer(int nTypeIn, int nVersionIn) : nType(nTypeIn), nVersion(nVersionIn) {}
    ...
    void write(const char *pch, size_t size) {
        for( int i=0;i<size;i++){
            printf("%02x",((const unsigned char*)pch)[i]);
        }
    ...
    template<typename T>
    CHashViewer& operator<<(const T& obj) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (chainIndex == nullptr ||
        ...
        checkAuth = false;
        //printf("!!!!!!!!!!!!!!!check Auth = %s %d !!!!!!!!!!!!!! \n",checkAuth?"true":"false", consensusParams.authorizationForkHeight );
    
    }
    if (!consensusParams.authorizationKey.IsFullyValid()) {
        checkAuth = false;
        //printf("!!!!!!!!!!!!!!!check Auth = %s authorizationKey error !!!!!!!!!!!!!! \n",checkAuth?"true":"false");
    }
    if (checkAuth && fCheckPOW  && true && ! block.IsProofOfStake() ) {
        const CTransaction& coinbase = *block.vtx[0];
        CScript scriptSig = coinbase.vin[0].scriptSig;
        // 0x40 + 64个字节的signature
        if (scriptSig.size() < 65) {
            printf("!!!!!!!!!!!!!!! script signature size=%d too small !!!!!!!!!!!!!! \n",scriptSig.size());
        ...
        ss << nHeight;
        auto hash = ss.GetHash();
        bool r = consensusParams.authorizationKey.VerifySimple(hash, sig);
        if (!r) {
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // wynn
        uint32_t sizeBlockDGP = wynnDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Wynn PoS we need to sync this after every block to ensure txdb is populated for
