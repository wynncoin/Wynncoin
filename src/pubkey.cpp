...
//@wynn
bool CPubKey::VerifySimple(const uint256 &hash, const std::vector<unsigned char>& vchSig) const {
    std::vector<unsigned char> vch;
    unsigned char size = vchSig.size();
    if( size != 66 ){
        printf("==================\nsignature size must 66\n==================\n");
        return false;
    }
    bool trip = true;
    vch.push_back(0x30);
    ...
    for( int i = 0 ; i < 33 ; i ++ ){
        ...
        vch.push_back(vchSig[i]);
    }
    vch.push_back(02);
    vch.push_back(0x21);
    for( int i = 0 ; i < 33 ; i ++ ){
        if( trip ){
            if( vchSig[6+vch[3]+i] == 0 ){
                vch[1] --;
                vch[5+vch[3]] --;
                continue;
            }
        ...
        vch.push_back(vchSig[33+i]);
    }

    //for( int i = 0 ; i < size ; i ++ )
    //    vch.push_back(vchSig[i]);
    //printf("VerifySimple: complex>");
