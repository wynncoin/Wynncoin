...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setWynnSchedule(wynnDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = wynnDGP.getBlockSize(nHeight);
    minGasPrice = wynnDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = wynnDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    WynnTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractWynnTX resultConverter;
    if(!convert.extractionWynnTransactions(resultConverter)){
    ...
    for(WynnTransaction wynnTransaction : wynnTransactions){
        txGas += wynnTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + wynnTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(wynnTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, wynnTransactions, hardBlockGasLimit);
...
bool CheckStake(const std::shared_ptr<const CBlock> pblock, CWallet& wallet)
{
    ...
    if( chainActive.Tip()->nHeight < Params().GetConsensus().nFirstPosBlock ){
        return error("CheckStake() : proof-of-stake not start");
    }
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "wynnstake";
...
void StakeWynns(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
