...
void checkResult(bool isCreation, std::vector<WynnTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    WynnTxConverter converter(transaction, NULL);
    ExtractWynnTX wynnTx;
    BOOST_CHECK(converter.extractionWynnTransactions(wynnTx));
    std::vector<WynnTransaction> result = wynnTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(wynntxconverter_tests, TestingSetup)
