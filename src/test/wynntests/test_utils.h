...
inline void initState(){
    ...
    const std::string dirWynn = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<WynnState>(new WynnState(dev::u256(0), WynnState::openDB(dirWynn, hashDB, dev::WithExisting::Trust), dirWynn + "/wynnDB", dev::eth::BaseState::Empty));
...
inline dev::Address createWynnAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline WynnTransaction createWynnTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    WynnTransaction txEth;
    if(recipient == dev::Address()){
        txEth = WynnTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = WynnTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<WynnTransaction> txs){
