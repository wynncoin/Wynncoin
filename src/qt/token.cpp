...
bool ToHash160(const std::string& strWynnAddress, std::string& strHash160)
{
    CTxDestination wynnAddress = DecodeDestination(strWynnAddress);
    if(!IsValidDestination(wynnAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&wynnAddress);
    if(keyid){
...
bool ToWynnAddress(const std::string& strHash160, std::string& strWynnAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination wynnAddress = keyid;
    if(IsValidDestination(wynnAddress)){
        strWynnAddress = EncodeDestination(wynnAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToWynnAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToWynnAddress(tokenEvent.receiver, tokenEvent.receiver);
