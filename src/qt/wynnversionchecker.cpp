...
#define paternVersion "wynn-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

WynnVersionChecker::WynnVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

WynnVersionChecker::~WynnVersionChecker()
{

}

bool WynnVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> WynnVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(WYC_RELEASES)));
    QEventLoop event;
...
Version WynnVersionChecker::getMaxReleaseVersion()
{
