...
#include <wynn/wynnstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

WynnState::WynnState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = WynnState::openDB(_path + "/wynnDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

WynnState::WynnState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute WynnState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, WynnTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createWynnAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            wynn::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> WynnState::vins() const // temp
{
...
void WynnState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* WynnState::vin(dev::Address const& _a) const
{
    return const_cast<WynnState*>(this)->vin(_a);
}

Vin* WynnState::vin(dev::Address const& _addr)
{
...
// void WynnState::commit(CommitBehaviour _commitBehaviour)
// {
...
void WynnState::kill(dev::Address _addr)
{
...
void WynnState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address WynnState::createWynnAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void WynnState::deleteAccounts(std::set<dev::Address>& addrs){
...
void WynnState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void WynnState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
