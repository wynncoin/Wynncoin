#include <wynn/wynnDGP.h>

void WynnDGP::initDataEIP158(){
...
bool WynnDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule WynnDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t WynnDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t WynnDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t WynnDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t WynnDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool WynnDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void WynnDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void WynnDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void WynnDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void WynnDGP::createParamsInstance(){
...
dev::Address WynnDGP::getAddressForBlock(unsigned int blockHeight){
...
void WynnDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void WynnDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void WynnDGP::parseStorageOneUint64(uint64_t& value){
...
void WynnDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule WynnDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
