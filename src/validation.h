...
#include <wynn/storageresults.h>


extern std::unique_ptr<WynnState> globalState;
...
using ExtractWynnTX = std::pair<std::vector<WynnTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // wynn
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashWynnRoot,
    const dev::h256& oldHashStateRoot, const std::vector<WynnTransaction>& transactions);
...
class WynnTxConverter{

public:

    WynnTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionWynnTransactions(ExtractWynnTX& wynnTx);
...
private:
    ...
    WynnTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<WynnTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
