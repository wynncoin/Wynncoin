...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/wynncoin>";
    const std::string URL_WEBSITE = "<https://wynncoin.net>";
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                dev::eth::Ethash::init();
                fs::path wynnStateDir = GetDataDir() / "stateWynn";
                bool fStatus = fs::exists(wynnStateDir);
                const std::string dirWynn(wynnStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsWynnstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<WynnState>(new WynnState(dev::u256(0), WynnState::openDB(dirWynn, hashDB, dev::WithExisting::Trust), dirWynn, existsWynnstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::wynnMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(wynnStateDir.string()));
                if (fReset) {
