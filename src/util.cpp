...
const char * const BITCOIN_CONF_FILENAME = "wynn.conf";
const char * const BITCOIN_PID_FILENAME = "wynnd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Wynn
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Wynn
    // Mac: ~/Library/Application Support/Wynn
    // Unix: ~/.wynn
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Wynn";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Wynn";
#else
    // Unix
    return pathRet / ".wynn";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Wynn Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Wynn Core developers";
    }
