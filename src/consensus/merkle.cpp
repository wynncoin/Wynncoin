...
void sha256tx(unsigned char* out, const unsigned char* in, size_t blocks){
    CSHA256 sha;
    unsigned char buf[CSHA256::OUTPUT_SIZE];
    unsigned char buf2[CSHA256::OUTPUT_SIZE];
    unsigned char buf3[CSHA256::OUTPUT_SIZE];
    sha.Reset().Write(in, blocks).Finalize(buf);
    sha.Reset().Write(buf, CSHA256::OUTPUT_SIZE).Finalize(buf2);
